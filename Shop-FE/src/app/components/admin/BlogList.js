...
const BlogList = ({ history }) => {
  ...
  const recordsAfterPagingAndSorting = () => {
    return stableSort(filterFn.fn(blogs), getComparator(order, orderBy)).slice(
      page * rowsPerPage,
      (page + 1) * rowsPerPage
    );
  };
  ...
  return (
    <Fragment>
      <MetaData title={`Tất cả bài viết - Admin`} />
      <SideBar />
          ...
          <section className="recent">
            <div className="table-grid">
              <div className="table-card">
                <h3 id="productListHeading">Danh sách bài viết</h3>
                <Link to="/admin/blog" className="link-add">
                  <Button className="action-btn-add">
                    <AddIcon /> Thêm bài viết
                  </Button>
                </Link>
